...
def resize_foreground(
    ...
    ratio = min(max(float(ratio), 1.0e-3), 1.0)
    alpha = np.where(image[..., 3] > 0)
    if alpha[0].size == 0 or alpha[1].size == 0:
    ...
    new_size = max(size, int(math.ceil(new_image.shape[0] / ratio)))
...
def _mesh_quality_score(mesh: trimesh.Trimesh) -> float:
    ...
    score = float(total_faces) + 0.5 * float(largest_faces) - 2.0 * float(fragment_faces)
    if mesh.is_watertight:
        score += max(10000.0, float(total_faces))
    score -= 5000.0 * max(0, len(parts) - 1)
    return score


def _filter_disconnected_fragments(mesh: trimesh.Trimesh) -> trimesh.Trimesh:
    parts = [
        part
    ...
    parts = sorted(parts, key=lambda part: len(part.faces), reverse=True)
    largest = parts[0]
    largest_faces = max(len(largest.faces), 1)
    largest_volume = max(abs(float(largest.volume)), 1.0e-9)
    largest_extent = max(float(np.max(largest.bounding_box.extents)), 1.0e-9)

    kept = [largest]
    for part in parts[1:]:
        face_ratio = len(part.faces) / largest_faces
        volume_ratio = abs(float(part.volume)) / largest_volume
        extent_ratio = float(np.max(part.bounding_box.extents)) / largest_extent
        if face_ratio >= 0.02 or volume_ratio >= 0.01 or extent_ratio >= 0.1:
            kept.append(part)

    filtered = kept[0] if len(kept) == 1 else trimesh.util.concatenate(kept)
...
def repair_mesh(
    ...
    original = mesh.copy()
    if len(original.vertices) == 0 or len(original.faces) == 0:
        return original

    original.remove_unreferenced_vertices()
    original.merge_vertices()
    trimesh.repair.fix_normals(original, multibody=True)

    raw_parts = [
        part for part in original.split(only_watertight=False)
        if len(part.vertices) > 0 and len(part.faces) > 0
    ]
    repaired = _filter_disconnected_fragments(original)
    if len(repaired.vertices) == 0 or len(repaired.faces) == 0:
        return repaired

    max_extent = float(np.max(repaired.bounding_box.extents))
    if max_extent <= 0.0:
        return repaired

    aspect_ratio = max_extent / max(
        float(np.min(repaired.bounding_box.extents)),
        1.0e-6,
    )
    convex_hull = repaired.convex_hull
    hull_fill_ratio = abs(float(repaired.volume)) / max(
        abs(float(convex_hull.volume)),
        1.0e-9,
    )
    should_solidify = (
        (not repaired.is_watertight)
        or len(raw_parts) > 1
        or (hull_fill_ratio >= 0.5 and aspect_ratio <= 1.6)
    )
    if not should_solidify:
        return repaired

    voxel_resolution = max(64, int(voxel_resolution))
    pitch = max_extent / float(voxel_resolution)

    try:
        voxelized = repaired.voxelized(pitch)
        solid = voxelized.fill()
    except Exception:
        return repaired

    close_iterations = [0]
    if hull_fill_ratio >= 0.7 and aspect_ratio <= 1.25:
        close_iterations.extend([1, 2, 3])
    elif hull_fill_ratio >= 0.5 and aspect_ratio <= 1.6:
        close_iterations.extend([1, 2])
    elif hull_fill_ratio >= 0.3 and aspect_ratio <= 2.2:
        close_iterations.append(1)

    candidates = [repaired]
    structure = np.ones((3, 3, 3), dtype=bool)
    for iteration_count in close_iterations:
        matrix = solid.matrix
        ...
        solid_mesh.apply_transform(solid.transform)
        solid_mesh.remove_unreferenced_vertices()
        solid_mesh.merge_vertices()
        trimesh.repair.fix_normals(solid_mesh, multibody=True)
        solid_mesh = _filter_disconnected_fragments(solid_mesh)
        candidates.append(solid_mesh)

    prefer_simple_topology = hull_fill_ratio >= 0.5 and aspect_ratio <= 1.6

    def candidate_score(candidate: trimesh.Trimesh) -> float:
        score = _mesh_quality_score(candidate)
        if prefer_simple_topology:
            score -= 7000.0 * abs(int(candidate.euler_number) - 2)
        return score
